...
class ParticleNetwork {
  constructor(canvasId) {
    ...
    this.ctx = null;
    this.particles = [];
    this.mouse = { x: null, y: null, radius: 200, isActive: false, antigravityRadius: 180, antigravityForce: 2.5 };
    ...
    this.config = {
      ...
      colors: {
        ...
        // Hilo tensado (telar): se tiñe de oro cuando ambos extremos caen
  ...
  drawLines() {
    const mouseActive = this.mouse.x !== null && this.mouse.isActive;
...
class Particle {
  ...
  update(mouse) {
    ...
    // Antigravity push: el cursor "empuja" las partículas
    if (mouse.x !== null && mouse.isActive) {
      const dx = this.x - mouse.x;
      const dy = this.y - mouse.y;
      const dist = Math.sqrt(dx * dx + dy * dy);
      const antigravityRadius = mouse.antigravityRadius || 180;
      const antigravityForce = mouse.antigravityForce || 2.5;

      if (dist < antigravityRadius) {
        const force = (antigravityRadius - dist) / antigravityRadius;
        const angle = Math.atan2(dy, dx);
        this.x += Math.cos(angle) * force * antigravityForce;
        this.y += Math.sin(angle) * force * antigravityForce;
      }
    }
