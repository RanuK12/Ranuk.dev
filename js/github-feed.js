...
(function () {
        ...
        container.innerHTML = top.map(l => {
            ...
            return `
                <div class="lang-row">
                    ...
                    <div class="lang-bar"><div class="lang-bar-fill" style="width:${width}%; background:${color}"></div></div>
                </div>
            `;
        }).join('');
    }
            ...
            return `
                <a href="${escapeHTML(r.url)}" target="_blank" rel="noopener" class="lab-repo-card">
                    ...
                    <div class="lab-repo-meta">
                        ${r.language ? `<span class="lab-repo-lang"><span class="lang-dot" style="background:${color}"></span>${escapeHTML(r.language)}</span>` : ''}
    ...
    function renderFootnote(generatedAt) {
        ...
        el.innerHTML = `<i class="fas fa-sync-alt"></i> ${word} ${ago} · data/github.json`;
    }
    ...
    async function load() {
        try {
            ...
            console.warn('[open-lab] no se pudo cargar github.json', err);
            const container = document.getElementById('lab-repos-list');
            if (container) container.innerHTML =
                '<p class="lab-error">No se pudo cargar el snapshot de GitHub. <a href="https://github.com/RanuK12" target="_blank">Visitá el perfil</a>.</p>';
        }
    }
